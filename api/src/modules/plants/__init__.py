...
async def plants(session: GetSession, limit: Annotated[int, Query(gt=0)] = 10, page: Annotated[int, Query(gt=0)] = 1):
    return await plant_manager.paginated_list(session, page, limit)


@r.get("/{id}", response_model=PlantCard, responses={404: {"model": ErrorModel, "detail": "Not found"}})
async def plant(session: GetSession, id: UUID):
    return await plant_manager.get_or_404(session, id=id, options=joinedload(Plant.note))
