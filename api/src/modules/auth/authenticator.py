...
class Authenticator:
    ...
    def __call__(self, options: list | None = None, is_verified: bool = False, superuser: bool = False):
        ...
        async def wrapped(
            ...
            stmt = stmt.join(Token, Token.user_id == User.id).where(
                (Token.token == token) & (Token.user_id == jwt['sub']))
            try:
                result = (await session.exec(stmt)).unique()
...
authenticated = Annotated[User, Depends(authenticator())]
